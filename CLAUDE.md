...
**残っている問題: ドッキング視点で構造物が白く飽和して形が読めない。**

| | |
| --- | --- |
| 描画されているか | **されている。** 判定 OFF / ON の対照で **921,600 画素中 917,813 画素（99.6 %）が変わる**（最大差 249 / 1280x720 / `earth-close-night`） |
| 見え方 | 窓の中が一様な白。**構造物の形も目印も読めない** |
| 場面依存か | **しない。** `cockpit-view` / `earth-close-night` / `sun-hidden` / `station-close` の 4 場面とも同じ |
| 幾何としては正しい | Scale 0.002 で全長 125 m・全幅 84 m の構造物の先端から 15 m。太陽電池アレイが視野を覆う |
| **原因は特定していない** | 照明・トーンマップ・bloom のどれが効いているか切り分けていない |

**この状態では平面モニタでの判定に使えない。** 実機（VR）で同じかは未確認。
候補（**選んでいない**）: 標準停止距離を伸ばす / 判定リグ専用の露出か照明を持つ /
ポストプロセスを判定中だけ切る / 目印だけを最前面に描く。
