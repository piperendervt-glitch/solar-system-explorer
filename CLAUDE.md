...
#### 13-3b: 接続面と Scale を決めるための判定ビュー（**値は未決定**）
...
| 視点 | ドッキング（ポート正面 / **暫定 0.015 units = 15 m**）と 全景（全長を縦に収める） |
| F4 | 判定 Scale **0.001〜0.008 / 刻み 0.00025**（連続）/ 判定の視点 / 目印 3 種の個別 ON・OFF |
| 目印 | 船の断面枠 1.6075 x 1.6312 m / 候補 3 円（金・青・桃）/ 1 m グリッド |
| F1 | Scale・全長・全幅・半径・MinStandoff・候補 3 つの実寸と船幅比と成立 Scale 範囲・station-close の角直径と被覆 |

**標準停止距離 0.015 units は暫定値。** near clip 0.01 units の外にあることだけを満たす。
実際の `PortStandoff` は次のコミットで導出する。
