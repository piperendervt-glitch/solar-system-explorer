...
| Demo 4（ステーション） | 13-0〜13-3 | `step13-3` / `demo4` |
...
**Demo 4 は完了。タグ `demo4`。** 計画書は
[docs/plans/step13-demo4.md](docs/plans/step13-demo4.md)。ブランチは `demo4/station`（残してある）。

**13-4（遠景と発光）/ 13-5（ドッキング演出）/ 13-6（仕上げと実機確認）は実施しない。**
理由: **人間が指示した項目ではないため。** 計画書の該当 3 節は削除済み。
**この 3 ステップに回していた宿題は未着手のまま残る**（下の各「申し送り」を参照）。

**Demo 4 で入ったもの:** Cobble のステーションモデル（Scale 0.008）/ ポート定義（+Z / +Y）/
`StationDefinition` への定数の移設 / フレーム時間の計測 / bloom の既定値の変更 /
判定リグ（`-stationJudge`）。**入らなかったもの:** 航法灯・窓の発光・遠景の光点 LOD・誘導灯。
...
#### 13-0b: 巡航では遠景の距離帯を 1 フレームで通過する（**13-4 は実施しないので出番が無い**）

**0.9c は dt = 1/60 で 1 フレーム 4,496.887 units 進む。**
13-4a で予定していた遠景の距離帯（2,000 units / 200 units）は**1 フレーム未満で通過する**ので、
**巡航シナリオではこの帯の費用を 1 標本も取れない。**
**13-4 を実施しないので帯そのものが無い。** 後で距離帯を作ることがあれば、
基準値は**静止点**（例 500 units / 2,000 units）で取ること。
...
| 発光を持つマテリアル | **4 件だけ**（module6 / 7 / 10 / 11）。13-4 の窓・航法灯の制約になるはずだった（**13-4 は実施しないので使っていない**） |
...
**この幾何で決まる**が、**実際に何が遮っているかは測っていない**（13-3 / 13-4 で見る予定だった。**測らないまま終わった**）。
...
**発光を持つ 4 件の位置**（13-4 の窓・航法灯の制約になるはずだった。**13-4 は実施しないので使っていない**）:
...
#### 13-3b: 要求の原点をポート位置へ揃えた（2026-08-29）

**要求の判定を「構造物の中心から」から「ポート位置から」へ変えた。**

| | 原点 | 値 |
| --- | --- | ---: |
| AP の到着 | ポート位置（`station.PortPosition`） | 20 units（**据え置き**） |
| ドッキング要求（旧） | **構造物の中心** | 20 units |
| ドッキング要求（新） | **ポート位置** | **2.0 units** |

**原点が違うと下駄を履く。** 同じ位置でも中心から測ると
`PortLocalUnits + PortStandoff` ぶん遠い値になる（Cobble 0.21275 / 箱 0.3）。
ポート位置から 1.0 units の点は、中心からだと Cobble 1.21275 / 箱 1.3。
揃えたので **`RequestRange` は「ポートから何 units で要求できるか」になった。**

| 定義 | ポート位置にいるとき ポートから | 同 中心から |
| --- | ---: | ---: |
| Cobble | 0.0 | 0.21275 |
| 箱 | 0.0 | 0.3 |

**`RequestRange` は 20.0 -> 2.0（箱・Cobble とも）。** `DefaultRequestRangeUnits`。
20 は `UniverseConstants.ArrivalRadiusUnits` と同値で、**AP の値を流用していただけ**だった。
**2.0 は出発点であって確定値ではない。F4 で振って人間が決める。**

**AP の到着半径は 20 のまま。** 到着後に**手動で 18 units 詰める区間**が残るのは
意図した設計（計画書「到着後にひと呼吸ある形を残す」）。

`UndockDistance = RequestRange x 1.2` なので 24.0 -> **2.4**。
補間は距離に依らず **`DockSeconds = 5.0` 秒固定**で、変わるのは詰める速さ
（20 units なら 4.0 km/s 相当、2.0 units なら **0.4 km/s 相当**）。

**F4 に「要求可能距離 [units]」を足した**（0.2〜20.0 / 刻み 0.1 / 既定 2.0）。
F1 には「ポートまで [units / m]」「要求可能」「圏内 / 圏外」「出港距離」
「補間の秒数と詰める速さ」を出す。

#### 13-3b: **平面 36 枚が 0 px にならなかった（原因は特定済み）**

**8 / 36 枚が変わった。すべて `stars` の視点。**

| 画像 | 差 | 最大差 |
| --- | ---: | ---: |
| `stars_01_normal` ほか 6 枚 | 327 px | 183 |
| `stars_12_SwapOverlayOrder` | 222 px | 149 |
| `cockpit-view` / `proxy` の 24 枚 | **0 px** | 0 |

**雑音の下限は 0 px**（同じコードで 2 回撮って確認）。**実変化。**

**中身は計器の文字。** 差のあった画素は 1280x720 の x 742..808 / y 566..572 に
固まっており、**計器の DOCK 欄が `APPROACHING` から `FREE` に変わった**もの。

`stars` の視点は `earth.AbsolutePosition + PortDirection * 20`（`XrStackCapture`）。

| | 距離 | 要求可能 | 状態 |
| --- | ---: | ---: | --- |
| 旧（中心から） | 20.000 | 20.0 | **境界ちょうど**（20 <= 20）で `Approaching` |
| 新（ポートから） | 19.787 | 2.0 | `Free` |

**旧の値は境界の上に乗っていた。** 20 units ちょうどで要求圏に入る位置だったので、
原点を 0.21275 ずらしただけで状態が反転した。**絵の作られ方は変わっていない**
（構造物・カメラ・ポストプロセスはどれも動いていない）。

**基準画像は新しいものに更新した。**

#### 13-3b: bloom の既定値を変えた（2026-08-29 / **人間が実機で判定**）

| 項目 | 旧（既定） | 新（既定） |
| --- | ---: | ---: |
| bloom 強度 | 3.00 | **0.05** |
| bloom しきい値 | 0.90 | **3.00** |
| bloom 拡散 | 0.70 | **0.35** |

**旧値は Demo 2 で太陽を見て決めた値**（2026-08-27 に実機で目視）。
「眩しさはしきい値を下げて滲ませるのではなく、コロナで面積を稼ぐ」
「0.25 まで下げると星空が白い塊に潰れ、計器の文字も光る」という実測が根拠だった。

**変えた理由: 13-3b でステーションが bloom を飽和させ、形も目印も読めなかった。**
発光を 1 つも足していない段階（`NavLights` も `WindowEmissives` も空）で
起きているので、計画書 §11 の「ステーション側の強度で吸収する」は成立しない。
飽和させているのは発光ではなく構造物そのもの。**13-4 で決め直す予定を前倒しした。**

**変更したのは `PlanetAppearance` のこの 3 定数だけ。**
太陽の発光強度 16.00 も 画面の発光強度 0.75 も**動かしていない**
（どちらも bloom 込みで決めた値なので、前提は変わっている）。

**F4 の `R`（全部リセット）は新値へ戻る。旧値には戻らない。**
旧値を試すときは F4 で手で入れること（強度 3.00 / しきい値 0.90 / 拡散 0.70）。

#### 13-3b: 基準画像を撮り直した（2026-08-29）

**手順は「旧値で撮る -> 3 定数だけ変える -> 新値で撮る」。**
対照を先に取ったので、この差が bloom の変更によるものだと後から疑わずに済む。
対照は [verify/xr-bloom-old/](verify/) に置いてある（追跡外）。

**変更は `PlanetAppearance.cs` の 3 定数のみ**（`git status` で 1 ファイル）。
`Assets/Materials/PostProcess.asset` の差分も 3 行だけ
（`0.9 -> 3` / `3 -> 0.05` / `0.7 -> 0.35`）。**他に原因は無い。**

**36 枚の変化（1280x720 / 差のあった画素）:** 合計 13,158,697 / 29,030,400 px = **45.3 %**

| 場面 | `01_normal` | 最大差 | 12 枚の範囲 |
| --- | ---: | ---: | --- |
| `cockpit-view` | 359,248 px (39.0 %) | 163 | 6.1 〜 59.1 % |
| `proxy` | 921,600 px (**100.0 %**) | 161 | 8.9 〜 100.0 % |
| `stars` | 81,424 px (8.8 %) | 57 | 0.1 〜 8.8 % |

`proxy` は太陽のコロナが画面を覆う視点なので全画素が動く。
`stars` は星空だけなので小さい。`02_colormap` / `03_mask` は 230,400 px の診断画像で、
**絵が変わったので測定結果も変わっている**（0.1 〜 19.1 %）。

**基準画像（`verify/xr/`）は新値のものに更新済み。**

#### 13-3b から 13-6 への申し送り: **確定値の前提が変わった（13-6 は実施しない = 未確認のまま）**

**Demo 1・Demo 2 で確定させた次の値は bloom 込みで決めたもの。**
bloom を変えたので前提が変わっている。**13-6 の実機確認で見る予定だったが、
13-6 は実施しないので、下の 4 項目は「前提が変わったまま未確認」で確定する。**
**値そのものは動かしていない**（太陽の発光 16.00 / 画面の発光 0.75 はそのまま）。
見え方が気になったときは、比較用のスクリーンショット（下）から入ること。

| 値 | 決めた場所 | いまの値 | 何が変わったか |
| --- | --- | ---: | --- |
| 太陽の発光強度 | Demo 2 (9-1) | 16.00 | bloom のしきい値が 0.90 -> 3.00 になり、滲む面積が減る |
| コロナ・光条・ゴースト | Demo 2 (9-2〜9-4) | — | 同上。眩しさの出方が変わる |
| 画面の発光強度 | Demo 3 (11-3b) | 0.75 | **しきい値 0.90 の下に置いた値。** 新しいしきい値 3.00 に対しては余裕が大きい |
| 地球の縁の滲み | Demo 2 (9-4) | — | 縁の青の比で測っていた（bloom 0.00 で 0.5369 / 3.00 で 0.4102） |

**比較用のスクリーンショット**（旧値・新値を上下に並べたもの）:
[docs/screenshots/bloom/](docs/screenshots/bloom/)

| 場面 | ファイル | 旧・新の差 |
| --- | --- | ---: |
| 太陽が写る | `compare_sun-hidden.png` | 559,211 / 2,073,600 px (27.0 %) |
| 計器の文字 | `compare_cockpit-view.png` | 973,956 / 2,073,600 px (47.0 %) |
| 明暗境界線 | `compare_earth-close-terminator.png` | 654,860 / 2,073,600 px (31.6 %) |

（1920x1080 / exe / `old_*.png` と `new_*.png` も個別に置いてある）

#### 13-3b-1: ポート・姿勢・配置を確定した（**人間が実機で判定**）

| 決めたこと | 値 | 出所 |
| --- | --- | --- |
| 接続面 | **(a) 中央の金色の円**（プレハブ単位 0.5241 m） | 人間が実機で判定 |
| `Scale` | **0.00800** | 同上。開口 4.193 m = 船の全幅 1.6075 m の **2.61 倍**（物差し 1.5〜3.0） |
| `PortLocal` | **(0.0300, 0.2400, 24.7182)** | 13-3a の実測。**Y は脊柱の軸上** |
| `PortForward` / `PortUp` | **+Z / +Y** | 2 軸で持つ（11-2c の縮退対策） |
| `PortStandoff` | **0.015 units = 15 m** | 導出値（下限 0.010 + 余裕 0.005） |
| `RequestRange` | **2.0 units**（確定） | 13-3b で原点をポート位置へ揃えたときに決めた。**当初は 13-5 で決める予定だった** |

**実寸（Scale 0.008 / 1 unit = 1 km）:** 全長 500.33 m / アレイ全幅 335.51 m /
リング径 198.86 m / 金の円 4.193 m / 半径 0.36062 units /
ポートは中心から 0.19775 units（197.7 m）前方。

**`MinStandoff` の式を差し替えた。**

| | 式 | Cobble での値 |
| --- | --- | ---: |
| 旧 | `RadiusUnits + nearClip` | 0.37062 units = **370.6 m** |
| 新 | **`HullAheadOfPortUnits + nearClip`** | 0.01000 units = **10.0 m** |

旧式は **`PortLocal = 0`（ポートが構造物の中心）を前提にした箱の式**で、
ポートが端にあるモデルでは過大になる（直径 4.19 m の口の 371 m 手前で停まる）。
新式の `HullAheadOfPort` は**ポートより前方への構造物のはみ出し**で、
箱は半径 0.25（ポートが中心なので半径ぶん出る）、Cobble は **0**。
**箱の値は変わらない**（0.25 + 0.01 = 0.26）。

制約の導出: 目はポート面から `PortStandoff` 前方にいるので、
最近傍の構造物までは `PortStandoff - HullAheadOfPortUnits`。これが near clip 以上。

| 寸法 | 値 | 出所 |
| --- | ---: | --- |
| near clip | 0.010 units = 10 m | Nearfield 段 |
| はみ出し | 0 | 13-3a（z > 24.7182 の頂点 0 件） |
| 下限 | 0.010 units = 10 m | 上の 2 つ |
| 余裕 | 0.005 units = 5 m | 補間の行き過ぎで構造物が消えないように |
| `PortStandoff` | **0.015 units = 15 m** | 下限 + 余裕 |
| ドッキング後の隙間 | 0.015 units = 15 m | `PortStandoff - はみ出し` |
| 機首からポート面まで | **10.68 m** | 15.0 - 4.3191（機首は目の 4.3191 m 前） |

**near clip 10 m のほうが機首の 4.3191 m より遠いので、機首とポート面の間には
必ず 5.68 m 以上の隙間が残る。** この構成では船が構造物に接触する絵は出せない。

**接近円錐のプローブ**（[verify/station-approach.txt](verify/station-approach.txt) /
`.	oolsun_unity.ps1 -Method SolarSetup.ProbeStationApproach`）:

| | |
| --- | --- |
| 円錐 | ポート面から `PortForward`(+Z) に半頂角 **30 度** / 長さ 15 m |
| 調べた頂点 | 30,076 |
| **円錐の中の頂点** | **0 件** |
| ポート面より前方の最近点 | **無し**（`HullAheadOfPort = 0` の裏付け） |

**S2 の宿題「MinStandoff の球の仮定を実際の幾何へ置き換える」はこれで片付いた。**

#### 13-3b-2: 受け入れ（Demo 1 の往復）は通った

PlayMode の `地球ステーションから火星ステーションへ着けて往復できる` が
**Cobble の定義のまま通る**（132 件 / failed 0）。実測:

| | 地球St -> 火星St | 火星St -> 地球St |
| --- | ---: | ---: |
| 所要 | 299.13 秒 | 300.47 秒 |
| 最終距離（中心から） | **0.2128 units** | **0.2128 units** |
| 最終速度 | 0.0000 km/s | 0.0000 km/s |

**0.2128 = ポート位置 0.19775 + PortStandoff 0.015。** 設計値どおり。

状態の遷移（両方向とも）:
`AP:Align -> AP:Cruise -> AP:Brake -> AP:Arrived -> AP:Idle ->
DOCK:Approaching -> DOCK:DockRequested -> DOCK:Docking -> DOCK:Docked`

**到着 -> AP 解除 -> ポート正面へ機首 -> 要求**の手順が成立している。
出港も 5.02 秒で `Free`。

#### 13-3b-2: AP の到着半径と `RequestRange` の関係（**S2 の宿題**）

**2 つは別の原点から測っている。**

| | 測る対象 | origin | 値 |
| --- | --- | --- | ---: |
| AP の到着 | `AutopilotSolver.RemainingDistance` | **ポート位置**（`station.PortPosition`） | `UniverseConstants.ArrivalRadiusUnits` = 20 |
| ドッキング要求 | `station.DistanceFrom(ship)` | **構造物の中心** | `Definition.RequestRange` = 20 |

**原点の差 = `PortLocalUnits` + `PortStandoff`。**
Cobble は 0.19775 + 0.015 = **0.21275 units**（箱は 0 + 0.3 = 0.3）。

したがって AP が `Arrived` になった瞬間の「中心からの距離」は最大で
**20 + 0.21275 = 20.21275 units** になりうる。**`RequestRange` 20 をわずかに超える。**
実測では船が惰性で内側へ入るため要求は通っている（上の往復）が、**余裕は 0.21 units しかない。**

**`RequestRange` を 20 より小さくすると、AP は 20 で止まったまま要求が通らなくなる。**
`UniverseConstants.ArrivalRadiusUnits` は `AutopilotSolver` の 5 箇所
（`Engage` の制動速度 / `Cruise` の制動開始 / `StepBrake` の 2 箇所 / `UpdateEta`）
から読まれており、**定義側の値を見ていない。**

**必要な関係:** `RequestRange >= ArrivalRadiusUnits + PortLocalUnits + PortStandoff`
を満たすか、**AP が対象ステーションの `RequestRange` を読むように直すか**のどちらか。

**その後（同日）: どちらも採らなかった。** 代わりに**要求の原点をポート位置へ揃え**、
`RequestRange` を **2.0 units** にした（上の「要求の原点をポート位置へ揃えた」）。
**AP の到着半径 20 はそのまま**なので、到着してから**手動で 18 units 詰める区間が残る。**
これは意図した設計で、`ManualFlightPlayModeTests` の往復がその手順で通っている。
**上の表の「ドッキング要求」の行は原点を揃える前の値**（中心 / 20）。

#### 13-3b-2: `RequestRange` の候補（**(b) 2.0 に確定**）

Cobble の寸法: 半径 **0.36062 units** / 全長 **0.50033 units** /
リング径 **0.19886 units** / ポート面は中心から **0.19775 units**。

| 候補 | 値 [units] | 実寸 | 半径の | 全長の | 出港距離 (x1.2) | AP をどうするか |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| (a) 現状維持 | 20.0 | 20 km | 55.5 倍 | 40.0 倍 | 24.0 units | **変更不要** |
| (b) | 2.0 | 2 km | 5.5 倍 | 4.0 倍 | 2.4 units | AP の到着半径も下げる必要がある |
| (c) | 1.0 | 1 km | 2.8 倍 | 2.0 倍 | 1.2 units | 同上 |
| (d) | 0.5 | 500 m | 1.4 倍 | 1.0 倍 | 0.6 units | 同上 |

**判断の材料にする予定だった 13-5 の誘導灯**（計画書「ポート正面の線上に等間隔の発光球、
**Standoff の外から点灯**」）**は実施しないので、この表は使われなかった。**
Standoff は 0.015 units（15 m）なので、灯の列は 0.015 から `RequestRange` まで伸びるはずだった。

| 候補 | 灯の列の長さ | Standoff の | 構造物の全長の |
| --- | ---: | ---: | ---: |
| (a) 20.0 | 19.985 units = 20.0 km | 1332 倍 | 39.9 倍 |
| (b) 2.0 | 1.985 units = 2.0 km | 132 倍 | 4.0 倍 |
| (c) 1.0 | 0.985 units = 985 m | 66 倍 | 2.0 倍 |
| (d) 0.5 | 0.485 units = 485 m | 32 倍 | 1.0 倍 |

**確定は (b) 2.0 units。** 誘導灯（13-5）を実施しないので、灯の列の長さは判断に効いていない。
決め手は**原点をポート位置へ揃えたこと**で、20 は `ArrivalRadiusUnits` を流用していただけの値だった。

#### 13-3b-2: `station-close` は別の場面になった（**場面は凍結**）

**座標は動かしていない。新しい場面も足していない。**
13-0b の 5 場面のうち、`station-close`（D = 0.5 units）だけ性質が変わった。

| | 箱（〜13-3b-1 の前） | Cobble（Scale 0.008） |
| --- | ---: | ---: |
| 半径 [units] | 0.25 | **0.36062** |
| 角直径（外接球 / D=0.5） | 53.13 度 | **71.60 度** |
| 画面に占める割合 | 33.13 % | **68.94 %** |

（条件: 1920x1080 / 縦画角 60 度 / 角直径 = 2·atan(R/D) /
被覆 = π·(f·tan(θ/2))² / (W·H)、f = (H/2)/tan(FOV/2)）

**71.60 度は縦画角 60 度を超えるので、外接球としては画面に収まらない。**
ただし**実際のシルエットは外接球より小さい。** 実行時の実測（exe / 1280x720）:

| | 値 |
| --- | --- |
| カメラからの bbox 中心 | (-0.00198, **-0.55242**, -0.00024) |
| bbox サイズ | (0.19871, 0.50033, 0.33551) |
| 手前の端 / 奥の端 | 0.302 units / 0.803 units |

外接球 0.36062 は**細長い構造物の対角**なので、正面から見た見かけより大きく出る。
**角直径 71.60 度は「収まらない」ことの目安であって、シルエットの実測ではない。**

`ScenarioTests.ステーションが視線を塞がない` からは 12-2b の時点で
`station-close` を名指しで除外済みなので、テストは落ちない。

#### 13-3b-1: 静止画に写らなかったのも bloom（**対照つきで確定**）

`station-close` の exe スクショで**ステーションが 1 画素も見えなかった。**
切り分けの実測:

| 測ったこと | 結果 |
| --- | --- |
| シーンの実体 | プレハブ 16 レンダラー / すべて enabled / レイヤー 10 |
| `Cam_Nearfield` | enabled / mask 1024（レイヤー 10 を含む）/ near 0.01 / far 100 |
| 実行時の位置 | カメラ (0,0,0) から見て bbox 中心 (-0.002, **-0.552**, -0.000) / サイズ (0.199, 0.500, 0.336) |
| 実行時の可視 | **16 / 16 レンダラーが `isVisible`**（frame 200） |
| **対照 `-noPost`** | **ステーションがはっきり写る** |

**原因は bloom。** 描画も配置も正しく、ポストプロセスが飽和させていた。
診断用に **`-noPost`**（ポストプロセスを切って起動）を足した。**無指定なら何も変わらない。**

#### 13-3b: 接続面と Scale を決めるための判定ビュー（判定は完了）
...
| 視点 | ドッキング（ポート正面 / F4 で **1〜200 m** を連続。初期値は暫定 0.015 units = 15 m）と 全景（全長を縦に収める） |
| F4 | 判定 Scale **0.001〜0.008 / 刻み 0.00025**（連続）/ **判定 距離 0.001〜0.200 units / 刻み 0.001**（連続）/ 判定の視点 / 目印 3 種の個別 ON・OFF |
...
**ドッキング距離の下限は 0.001 units（1 m）。near clip の内側まで寄れる。**

**一度は下限を near clip（0.010 units = 10 m）にしていたが、外した。**
実機で振った人間の判断で、**判定に使うには近づけ方が足りなかった。**
near clip の内側ではモデルの手前側が描かれないが、
**船とステーションの大小を見る**にはそれでも寄りたい、というのが理由。

**ゲーム本体の near clip は変えていない。** 変えたのは判定リグが取れる距離の範囲だけ。
`NearfieldNearClipUnits`（0.01）は「いま内側に入っているか」を F1 に出すために残してある。
`ClampDockingDistance` の役目は **0 と負を弾くことだけ**になった。

**刻みは一律 0.001 units（199 段）。** 上限まで押す回数は増える。
上側だけ粗くする案は今回採らない（使ってみて不便なら直す）。

**機首からの隙間 = 目からの距離 [m] - 4.3191 m。**
機首は目（`EyeLocal (0, 0.429, -1.436)`）の 4.3191 m 前にある（13-3a の実測）。
目からの距離だけを見ると船の長さを頭の中で引き算することになるので、両方を出す。
**下限を下げたので隙間が負になる領域が目盛に入った**（機首がポート面より先にある状態）。

| 目からの距離 [units] | [m] | 機首からの隙間 [m] | |
| ---: | ---: | ---: | --- |
| 0.001（下限） | 1.0 | **-3.3191** | 機首がポート面より先 |
| 0.0043191 | 4.3 | 0 | 機首がポート面に接する |
...
#### 13-3b から 13-4 への申し送り: **bloom の前提が崩れていた（13-3b で決め直し済み）**
...
**始末: 13-4 を待たず 13-3b で bloom の設定自体を決め直した**
（強度 3.00 -> **0.05** / しきい値 0.90 -> **3.00** / 拡散 0.70 -> **0.35**。
人間が実機で判定）。**13-4 は実施しないので、この申し送りは 13-3b で閉じている。**

**ただし惑星と計器の見え方への影響は残る。** 旧値は Demo 2 で太陽を見て決めた値で、
36 枚のうち 45.3 % の画素が動いた。**撮り比べは済んでいる**（→ 上の比較スクリーンショット）が、
**実機で見直す 13-6 は実施しない。** → 上の「13-6 への申し送り」。

#### 13-2 から 13-4 への宿題: 4K テクスチャ 46 枚と VRAM（**13-4 は実施しない = 下げないまま**）
...
**13-4（遠景と発光）で実測してから決める予定だったが、13-4 は実施しない。**
**下げないまま確定する。** VRAM を測っていないので、**効くかどうかも分かっていない。**
後で必要になったら、まず VRAM の実測から入ること（見積もりの数字をここに書かない）。
...
- **bloom は 13-3b で決め直した（2026-08-29）。強度 0.05 / しきい値 3.00 / 拡散 0.35。**
  旧値 3.00 / 0.90 / 0.70 は Demo 2 で太陽を見て決めた値で、Demo 3 では変えない方針だった。
  **ステーションが bloom を飽和させたので前倒しで変更した**（→ §0 の 13-3b）。
...
| 画面の発光強度 | **0.75**（`CockpitDefinition.DefaultScreenEmission`） | 11-3b。**決めた当時の** bloom しきい値 0.90 の下（しきい値は 13-3b で 3.00 へ。値は動かしていない） |
...
**Demo 4（Step 13）で確定した値。** 上の 13-3b-1 の表（接続面 / `Scale` / `PortLocal` /
`PortForward` / `PortUp` / `PortStandoff`）とあわせてこれが全部。

| 決めたこと | 値 | 出所 |
| --- | --- | --- |
| ステーションのアセット | **Space Station Free（Cobble Games / 無料）** | 13-0。SF 路線。ISS 路線は採らなかった |
| `Scale` | **0.00800** | 13-3b-1。人間が実機で判定。開口 4.193 m = 船の全幅の 2.61 倍 |
| `PortStandoff` | **0.015 units = 15 m** | 13-3b-1。下限 0.010（near clip）+ 余裕 0.005 |
| **`RequestRange`（要求可能距離）** | **2.0 units**（`StationCatalog.DefaultRequestRangeUnits`） | 13-3b。**原点はポート位置。** AP の到着半径 20 とは別の値 |
| **出港距離 `UndockDistance`** | **2.4 units**（= `RequestRange` x 1.2） | 13-3b。`LeaveMultiplier` 1.2 は据え置き |
| AP の到着半径 | **20 units のまま**（`UniverseConstants.ArrivalRadiusUnits`） | 13-3b。到着後に**手動で 18 units 詰める区間**を残すのは意図した設計 |
| bloom 強度 / しきい値 / 拡散 | **0.05 / 3.00 / 0.35** | 13-3b。人間が実機で判定。旧値 3.00 / 0.90 / 0.70 |
| **航法灯・窓の発光** | **空のままで確定**（`NavLights` / `WindowEmissives`） | **13-4 を実施しないため。** 型と読む経路だけ残す。空を縛る番人テストも残す |
...
**項目は分類ごとに 1 画面ずつ出る (Step 13-3b 追補)。**
`↑` `↓` は今までどおり全項目を 1 列で辿り、**境目で分類が切り替わる。**
新しいキーは増やしていない（起動引数の口も変えていない）。

| 分類 | 件数 | 中身 |
| --- | ---: | --- |
| 段と天体 | **20** | 1 段だけ表示 / カメラ段 4 / 天体 3 x 3 / 雲・ステーション・スカイボックス・ポスト・フレア・コロナ |
| 見た目 | 13 | 大気・雲・フレア・太陽・コロナ・光条・ゴースト / bloom 3 |
| コックピット | 13 | 画面 4 / 補助光 2 / 計器の向き / 目 3 / 画角 / 音 / 微振動 |
| ステーションと判定 | 7 | ステーションの倍率 / 判定 6 |

**レイアウトは全項目数ではなく分類ごとの最大件数（20）で決まる。**
1080p の利用可能高 1056 px / 行高 = フォント + 6 / 固定 10 行（見出し 5 + 空行 1 + 天体 4）:

| | |
| --- | --- |
| いま | 最大の分類 20 -> `16 + (10 + 20) * 20 = 616 <= 1056` で**フォント 14**（分類にする前は 53 項目でフォント 10 = `MinFontSize`） |
| 1 つの分類に入る上限 | **42 項目まではフォント 14**（`16 + (10 + 42) * 20 = 1056`）。**55 項目まで窓表示にならない**（フォント 10） |
| 720p | 最大の分類 20 なら `16 + (10 + 20) * 16 = 496 <= 696` で**窓表示に落ちなくなった**（以前は落ちていた） |

**分類の無い id は例外にしてある**（`DebugPanelModel.CategoryOf`）。
項目を足したときに黙って落ちない。`DebugPanelLayoutTests` が
「全項目がどれかの分類に入っている」「カーソルを回すと全項目に到達する」を縛る。
