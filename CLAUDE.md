...
| | CPU 平均 [ms] | 実行間のばらつき | GPU の標本取得率 |
| --- | --- | ---: | --- |
| `captureFramerate = 60` | 1.554〜1.717（3 回） | **0.163** | 64〜74 % |
| 固定なし | 1.360〜1.378（3 回） | **0.019** | 81〜86 % |

**+15 % / ばらつき 8.6 倍 / GPU の取得率が低下。** 2 条件の範囲は重ならない。
別の日の別バッチでも +13〜15 % で再現する（水準そのものはバッチ間で ±0.2 ms 動くので、
**バッチをまたいだ絶対値の比較はできない**）。
