...
#### 13-3b から 13-6 への申し送り: **確定値の前提が変わった**

**Demo 1・Demo 2 で確定させた次の値は bloom 込みで決めたもの。**
bloom を変えたので前提が変わっており、**13-6 の実機確認で改めて見ること。**
...
| `RequestRange` | **20 units のまま** | **13-5 で決める。まだ動かしていない** |
...
**まだどちらもやっていない**（`RequestRange` は 20 のまま）。

#### 13-3b-2: `RequestRange` の候補（**選んでいない。人間が指定する**）
...
**判断の材料（13-5 の誘導灯）:** 計画書は
「ポート正面の線上に等間隔の発光球、**Standoff の外から点灯**」。
Standoff は 0.015 units（15 m）なので、灯の列は 0.015 から `RequestRange` まで伸びる。
...
**どれも「正しい」とは書かない。** 灯を何個・どの間隔で置くかが決まると
列の長さが決まるので、13-5 の設計と一緒に人間が決める。
