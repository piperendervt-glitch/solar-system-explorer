namespace SolarSystem.Core
{
    ...
    public static class PlanetAppearance
    {
        ...
        /// <summary>
        /// Bloom の強度。**3.00 で確定。ただし範囲の上限に張り付いている。**
        /// パネルの範囲を 0.0〜6.0 に広げてあるので、次に exe を触るときに
        /// 3.0 より上を試して、これで足りているのかを確かめる。        ///
        /// **実機で目視して決定 (2026-08-27)。**
        /// 眩しさは「しきい値を下げて多くの画素を滲ませる」のではなく、
        /// **コロナでしきい値を超える面積そのものを広げる**方向で解決した。
        /// しきい値を 0.25 まで下げると星空が白い塊に潰れ、計器の文字も光る。
        /// </summary>
        public const double BloomIntensity = 3.00;

        /// <summary>
        /// Bloom のしきい値。**0.90 で確定。**
        /// 太陽のトーンマップ前の出力は 9.055 なので、これを超えていれば滲む。
        /// **これ以上は下げない。** 0.25 まで下げると星空が白い塊に潰れ、
        /// 計器の文字まで光る。面積はコロナ側で稼ぐ。        ///
        /// **実機で目視して決定 (2026-08-27)。**
        /// 眩しさは「しきい値を下げて多くの画素を滲ませる」のではなく、
        /// **コロナでしきい値を超える面積そのものを広げる**方向で解決した。
        /// しきい値を 0.25 まで下げると星空が白い塊に潰れ、計器の文字も光る。
        /// </summary>
        public const double BloomThreshold = 0.90;

        /// <summary>Bloom の拡散。**0.70 で確定 (実機で目視)。**</summary>
        public const double BloomScatter = 0.70;
