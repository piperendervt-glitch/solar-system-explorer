...
**Demo 2 は完了。** 各 Step の完了時点に git タグを打ってある。

| Step | 内容 | タグ | 状態 |
| --- | --- | --- | --- |
| 8-0 | 共通の小物（F1 トグル・微振動・検証ハーネス） | `step8-0` | 完了 |
| 8-0b | F4 デバッグパネル | （タグ無し） | 完了 |
| 8-1 / 8-2 | テクスチャ取り込み・惑星シェーダ | `step8-1` | 完了 |
| 8-3 / 8-4 | 雲層・自転 | `step8-3` | 完了 |
| 8-5 | プロキシ殻と実スケールの整合 | `step8-5` | 完了 |
| **8-6** | **リムライト／大気散乱の作り込み** | — | **省略確定**（§7 のとおり。8-2 の大気の縁で足りると判断した） |
| 9-1 | 太陽の HDR 化 | `step9-1` | 完了 |
| 9-2 | コロナ | `step9-2` | 完了 |
| 9-3a | フレアの遮蔽判定 | （9-1 の前に前倒し） | 完了 |
| 9-3b | フレアの見た目（光条・縞・ゴースト） | `step9-3b` | 完了 |
| 9-4 | bloom と露出の再調整 | `step9-4` | 完了 |
| 10-1 / 10-2 | 素材取り込み・グループ構成 | `step10-1` | 完了 |
| 10-3 | エンジン音のスラスト連動 | `step10-3` | 完了 |
| 10-4 | イベント音 | `step10-4` | 完了 |
| 10-5 | ドッキングでの音の切替 | （10-2 に含めて実装） | 完了 |

テストは **EditMode 257 件 / PlayMode 89 件**が緑。

### 実機で「耳と目で決めた」値

**下の値はすべて exe を起動し、F4 のデバッグパネルで振って人が決めたもの。**
計算や測定から出た値ではないので、変えるときはまた実機で見る／聴くこと。
出所はすべて `Core/PlanetAppearance.cs` と `Core/AudioMix.cs`（唯一の定数置き場）。

| 値 | 決定値 | 決め方 | 備考 |
| --- | ---: | --- | --- |
| `_AtmosphereStrength` | 5.00 | **目**（9-4 後に再確認して据え置き） | |
| `_CloudOpacity` | 1.15 | **目**（9-4 後に再確認して据え置き） | |
| 太陽の発光強度 | 16.00 | **目** | 計画書 9-1 の見積もり 4〜8 は bloom 消灯下のものだった |
| コロナの大きさ（本体比） | 6.00 | **目** | 決定時は範囲上限。上限を 12.0 に広げてある |
| コロナの減衰（指数） | 3.00 | 据え置き | |
| フレア基準強度 | 0.60 | 据え置き（9-3a で確定） | |
| 光条の長さ / 本数 / 太さ | 1.50 / 6 本 / 0.10 | 据え置き | 本数は**画面上の本数**（要素数はその半分） |
| ゴーストの強さ | 1.25 | **目** | 0.40 では見つけられなかった |
| bloom 強度 | 3.00 | **目** | 決定時は範囲上限。上限を 6.0 に広げてある |
| bloom しきい値 | 0.90 | **目** | これ以上は下げない（0.25 まで下げると星空が白い塊に潰れる） |
| bloom 拡散 | 0.70 | **目** | |
| 微振動の振幅 | 1.500E-003 rad | **目**（8-0 で確定） | |
| 音量 Master / Engine / Cockpit / SFX | 1.00 / 0.10 / 0.05 / 0.25 | **耳** | Engine と Cockpit は「鳴っていることに気付かない」程度まで下げた |
| エンジンの追従（時定数） | 0.50 秒 | **耳** | |

**眩しさの作り方（決定）**: しきい値を下げて多くの画素を滲ませるのではなく、
**コロナでしきい値を超える面積そのものを広げた。** 太陽の角直径 8.7 px は
物理的に正しい値なので変えていない。
...
| 値 | 決定値 | 根拠 |
| --- | ---: | --- |
| エンジンのクロスフェード長 | 0.100 秒 | 段差比 85.18 → 3.108 |
| コックピットのループ長 | 8.0 秒 | 素材 0.954 秒では 1 秒周期の反復に気付く |
| 段差比の上限 | 8.0 | 理想 1 前後。加工前 85.18 に 1 桁の余裕 |
| エンジンの音量係数 | 0.36 / 1.00 / 0.86 | 計画書の 0.25 / 0.70 / 0.60 を**全開 1.0 で正規化** |
| ドッキングのカットオフ | 12000 ⇄ 2000 Hz | 対数補間・1.5 秒・両端で速度 0 |
| 警告の最小間隔 | 0.15 秒 | 連打の間引き |

### 代表カット

`docs/screenshots/demo2/` に 3 枚（1280x720 JPEG）。

| ファイル | 内容 |
| --- | --- |
| `01_earth-day.jpg` | 地球（昼側）: 4k テクスチャ・雲層・大気の縁 |
| `02_earth-terminator.jpg` | 明暗境界: 夜側の街灯り・海の鏡面反射 |
| `03_sun-over-limb.jpg` | 惑星の縁から昇る太陽: HDR ディスク・コロナ・光条 |
...
> **Step 8-0b でデバッグパネル（F4）を追加した。** 「目で決めるしかない値」を
> 実機で決められるようにするため。操作方法とキー割り当ては
> [CLAUDE.md](../CLAUDE.md) §0-C にある。ハーネスと同じく Demo 3 以降でも使う道具なので、
> 計画書には閉じ込めない。
...
- **光点と殻の両方に同じ強度を掛ける（決定）。** 航行範囲で太陽は LOD 帯（4〜8 px）を**横切る**ため。
  地球近傍 8.70 px（光点 α 0.000 / 殻 α 1.000）→ 中間点 6.85 px（0.287 / 0.713）→
  火星近傍 5.71 px（0.572 / 0.428）で、**火星側では光点のほうが優勢**になる。
  殻だけ HDR にすると航行中に太陽が連続的に暗くなり、9-4 の露出調整でそれを追いかける羽目になる
  - F4 の数値項目も **1 つで両方に効かせる**。別々に振ると LOD 帯で不連続になる
  - 連続性の目視は `lod-midpoint` の確認項目で見る（新規シナリオは足さない）
...
- **オクルージョンは使わない（決定）。**
  ...
  - **Step 8-2 で実例を実測した。** `earth-close-night` (地球の夜側を正面から見る)
    では太陽が地球の真後ろに来るため、フレアが**画面中央 38x38 px の円盤**として
    地球を透かして描かれる。切り分けの実測:

    | 操作 | 画面中央 (940-980, 520-560) の平均 |
    |---|---|
    | そのまま | (88.7, 86.1, 81.0) |
    | 地球のレンダラを無効化 | (90.8, 88.0, 82.6) — **残る** |
    | `LensFlareComponentSRP` を無効化 | **(0, 0, 0) — 消える** |

    昼側では光源がカメラの背後なのでフレアは出ない (中央の輝点は地球の鏡面
    ハイライトで、地球のレンダラを切ると消える)。明暗境界では光の向きが視線と
    直交するのでフレアの画面位置が視錐台の外に出る。
  - **Step 9-3a で実装済み (前倒し)。** 引き渡し率ではなく**解析的な遮蔽判定**にした。
    `Core/FlareOcclusion.cs` が角半径と角距離から遮蔽率 0.0〜1.0 を返し、
    `SunFlareController` が `intensity = 0.6 × (1 - 遮蔽率)` を毎フレーム更新する。
    深度バッファを使わないので惑星が ZWrite off のままで成立する。

    | 判定 | 条件 | 値 |
    |---|---|---|
    | 遮蔽なし | `d >= as + ab` | 0.0 |
    | 完全遮蔽 | `d <= \|ab - as\|` | `ab >= as` なら 1.0 / そうでなければ面積比 `(ab/as)^2` |
    | 途中 | その間 | `smoothstep` で補間 |

    実測 (シナリオ / 遮蔽率 / 強度): sun-hidden 1.000 / 0.000、
    sun-emerge 0.507 / 0.296、sun-clear 0.000 / 0.600。
    観測者が天体の内部にいるとき (`D <= R`) は角半径を π/2 に倒す (asin の定義域外)。
  - ~~代替案「引き渡し率でフレア強度を減衰させる」~~ → 上記で置き換え済み。 `CelestialBodyView` の
    `RealScaleBlend` が既にあるので、接近して惑星が画面を占めるほどフレアを弱める
- 視界外で自動的に消える（コンポーネント標準機能）

### 9-4 露出の再調整

- **【9-4 実施済み】太陽に bloom が効いていなかった。原因は特定・修正した。**
  `PostProcessPreset.Apply` を呼ぶのが `SceneBuilder`（Editor）だけで、**実行時に呼ぶ口が無かった**。
  しかも Editor 側の `Apply` は `VolumeProfile` アセットへ `SetDirty` / `SaveAssets` をしていなかったので、
  アセットには Bloom の既定値（`intensity 0` = 消灯）が残っていた。**Step 6 以来 bloom は一度も効いていなかった。**
  `PostProcessPreset.Awake` で必ず適用するようにし、値の出所を `Core.PlanetAppearance` に一本化した
  - **Step 6 の「Medium（bloom 0.80 / しきい値 1.05）で確定」は実行時には効いていない値だった。**
    3 案比較の実測値（Subtle / Medium / Strong の四隅の明るさ 6.24 / 3.58 / 0.61）は、`Apply` を呼んだのと
    同じ Editor セッションで撮ったため、**測定にだけ現れていた**
  - 実測（exe / 1920x1080 / sun-face）: bloom 0.00 と 0.80 で **152,532 画素（全体の 7.36%）が変化、最大差 44/255**。
    非黒画素は 929,395 → 931,013。sun-clear では 206,573 画素（9.96%）が変化
  - **訂正: 「`Camera.Render()` → RenderTexture では bloom の差が出ない」は誤りだった。**
    太陽（4px）では強度 0.00 と 0.80 で明部 52 画素と 52 画素、差 0 だったが、これは小さすぎて検出できなかっただけ。
    画面いっぱいの地球なら RT でもはっきり出る（`earth-close-day` の縁の青の比が bloom 0.00 で 0.5369、3.00 で 0.4102）
  - **Overlay 3 段（Near / Nearfield / Cockpit）の `renderPostProcessing = true` は実効的に効いていない。**
    URP はスタックの最後のカメラ（Cam_Cockpit）の設定で一度だけ適用するため。**有害でもないので今回は触っていない。**
    変えるなら、変更前後のスクショが画素で一致することを実測してから別コミットで
...
- AudioSource（2D、Loop）を Ship に 1 つ。制御は Core 側に `EngineAudioModel`（UnityEngine 非依存）を置き、入力（スラスト 0〜1、速度ダイヤル段、AP 状態）→ 出力（volume 0〜1、**pitch 0.9〜1.2**）を純粋関数で決める
  - **【10-3 実施済み】volume は「係数」で、全開 1.0 になるよう正規化した（決定）。**
    比率は下の 0.25 / 0.70 / 0.60 のままで、全体を 0.70 で割る（アイドル 0.36 / 全開 1.00 / 制動中 0.86）。
    理由: 10-2 の試聴時、エンジンは**この係数が掛かっていない状態**で鳴っており、その音を聴いて Engine = 0.10 が決まった。
    0.25〜0.70 をそのまま掛けると全開でも 0.07 にしかならず、**耳で承認した音量を下回る。**
...
- **`root` の `LookRotation(dir)` が今も必要かの再検討。** 光点が Quad から球に変わった経緯があり、ビルボードの必要性が疑わしい。Step 8-5 の位相ずれ（殻側だけに親の回転が乗る）の遠因。実施は別タスク
