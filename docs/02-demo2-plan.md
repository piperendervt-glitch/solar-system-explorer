...
- **【9-4 実施済み】太陽に bloom が効いていなかった。原因は特定・修正した。**
  ...
  - **`Camera.Render()` → RenderTexture の経路では bloom の差が出ない**（強度 0.00 と 0.80 で明部 52 画素と 52 画素、差 0）。
    トーンマップは同じ経路で効いているので、ポストプロセス自体は走っている。**bloom だけ出ない理由は未特定。** 滲みの確認は exe で行う
