...
- 巡航中の**微振動**: Cockpit 段のカメラ親に、スラスト量に比例した振幅のパーリンノイズ揺れ。ドッキング中は 0
  - **振幅は 0.3 mm 相当 = コックピット描画空間で 0.0003 unit。**
    `CockpitRenderScale = 1000` かつ `Size = 0.002 * 1000 = 2`（実寸 2 m）なので、
    **この空間では 1 unit = 1 m**。0.3 mm = 3e-4 unit。
    （旧記述の「0.3 unit」は 0.3 m にあたり 1000 倍過大だった）
