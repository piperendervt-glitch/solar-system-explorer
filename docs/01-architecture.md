...
- 判断が分かれた箇所は **2026-08-26 に全件確定済み**。各章末の「決定」表と §11 の索引が正。
  ID は `D-1` から `D-25`。**D-10 のみ Step 2 の実測後に確定**（Step 1 には影響しない）。
...
| --- | --- | --- | --- |
| 火星 ← 地球 | 17.9 arcsec | 18〜25 arcsec（衝） | **0.090 px** |
| 地球 ← 火星 | 33.7 arcsec | — | 0.168 px |
| 太陽 ← 地球 | 0.533° | 0.53° | 9.60 px |
| 太陽 ← 火星 | 0.350° | 0.35° | 6.30 px |

**火星は地球から 0.09 px。** メッシュで描く意味はまったくなく、光点にするしかない。
...
| --- | --- | --- | --- | --- |
| 太陽 | 1.436e9 | 3.589e8 | 1.794e8 | 8.972e7 |
| 地球 | 1.314e7 | 3.285e6 | 1.643e6 | 8.213e5 |
| 火星 | 6.991e6 | 1.748e6 | 8.739e5 | 4.370e5 |

**4 px でメッシュを出し始め、8 px で光点を消す（その間はクロスフェード）。**（D-7）

| 天体 | フェード開始 (4 px) | フェード完了 (8 px) | 通過にかかる時間 @0.1c |
| --- | --- | --- | --- |
| 火星 | 1.748e6 units | 8.739e5 units | 29.2 秒 |
| 地球 | 3.285e6 units | 1.643e6 units | 54.8 秒 |
| 太陽 | 3.589e8 units | 1.794e8 units | （常にメッシュ。太陽〜地球 1.496e8 < 1.794e8） |

**太陽は常にメッシュ側**になる。地球から見ても 9.6 px あるので、光点で済ませる場面がない。
...
stateDiagram-v2
  ...
  Braking --> Arrived : 速度 ~ 0 かつ 残距離 < 到着圏
...
| `Arrived` | 到着圏内で停止。`DockingSolver` へ引き渡す |
...
| D-10 | 「到着圏内」の定義 | **未確定。Step 2 の実測後に決める** | 「どこからステーションが絵として見え始めるか」を見てから決めたほうが根拠のある数字になる。Step 1 には影響しない | 3, 5 |
...
stateDiagram-v2
  [*] --> Free
  Free --> Approaching : 距離 < 到着圏 かつ 速度 < 接近上限
  Approaching --> Free : 距離 > 到着圏 x 1.2 (ヒステリシス)
...
| --- | --- | --- |
| `Free` | 初期状態 | 通常航行 |
| `Approaching` | 距離 < 到着圏（**D-10**）かつ速度 < 接近上限 | **速度条件が必須**。高速通過で誤って入らないため |
...
| D-10 | 4 | 「到着圏内」の定義 | **Step 2 の実測後に確定（本 Step では不要）** | 3, 5 |
...
**D-10 だけ本 Step では確定していない。** 影響は Step 3 / 5 なので Step 1 の妨げにはならない。
